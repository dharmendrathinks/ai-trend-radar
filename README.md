...
Slack gets a **developer shortlist of up to 10 LLM-assessed topics** that qualify for the main list. New developments, material updates, and due reminders share one limit and are ranked together using the existing Developer Priority order. Unchanged topics are omitted; unassessed discoveries and watch items stay in local reports. Selection uses the saved changes brief, so its existing `--top`/configuration limits still apply.

Each topic has a linked title, overall score plus impact/relevance/urgency breakdown, and short **Changed**, **For**, and **Practical impact** excerpts. Scores are out of 100 and retain their existing precision. Dividers separate topics; smaller context text carries the change label, evidence type, topic ID, and any reminder that was not rechecked. Source text is shortened locally without another model call. Collection/assessment gaps and a shared scoring/evidence note remain visible, with counts of additional shortlisted and excluded changes available locally.

When no shortlisted changes are available, Slack sends a brief status message. Disabled, incomplete, or unavailable assessment is stated explicitly; unassessed items never fill the shortlist. Full reports, release notes, and YouTube results are not uploaded. Full details and review/defer commands remain in `latest.brief.md` in your configured reports folder. Slack filtering does not change local reports or review acknowledgement.
...
Formatting updates apply to newly queued briefs. Previously queued messages retain their original content, and a formatting update does not resend a confirmed scan.
